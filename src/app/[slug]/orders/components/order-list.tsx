...
const OrderList = ({ orders }: OrderListProps) => {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {orders.map((order) => (
        <Card key={order.id} className="border-gray-400">
          <CardContent className="space-y-4 p-5">
            <div className="flex items-center gap-2">
              <div className="relative h-5 w-5">
                <Image
                  src={order.comercio.avatarImageUrl}
                  alt={order.comercio.name}
                  className="rounded-sm"
                  fill
                />
              </div>
              <p className="text-sm font-semibold">{order.comercio.name} | #{order.id}</p>
